...
/**
 * Moved from `server/scanner.ts`, which never imported Vite. Filename
 * conventions, route tree building, conflict diagnostics.
 *
 * One thing is new: every handler tuple's length is recorded. Hono's typed
 * overloads end at path plus ten handlers and degrade silently past that, and
 * the emitter is the only place that knows how many slots a registration will
 * actually have — so the count has to come out of the scan.
 */
export type Route = {
  /** Handler tuple length per method, or `null` when a spread makes it unknown. */
  arities: Partial<Record<Method, number | null>>;
  file: string;
...
export async function discoverRoutes(
  ...
  const middlewareCounts = await settleInOrder(
    middlewareFiles.map(validateMiddlewareModule)
  );
...
async function validateMiddlewareModule(file: string): Promise<number | null> {
  const source = await readFile(file, 'utf8');
  const ast = parseModule(file, source, 'route module');
  const middlewareStacks = new Map<string, number | null>();
  ...
  for (const statement of ast.body) {
    ...
    for (const declaration of statementDeclaration.declarations) {
      const arity = handlerStackArity(
        declaration.init,
        middlewareFactories,
        middlewareStacks
      );
      if (declaration.id.type === 'Identifier' && arity !== undefined) {
        middlewareStacks.set(declaration.id.name, arity);
      }
    }
  }
  ...
  for (const statement of ast.body) {
    ...
    for (const specifier of statement.specifiers) {
      ...
      if (names.exported === 'default' && middlewareStacks.has(names.local)) {
        count = middlewareStacks.get(names.local);
      }
    }
  }
...
async function readRouteExports(file: string): Promise<{
  ...
  const handlerStacks = new Map<string, number | null>();
  const handlerFactories = new Set<string>();
  const methodStackNames = new Map<Method, string>();
    ...
    if (statement.declaration?.type === 'VariableDeclaration') {
      for (const declaration of statement.declaration.declarations) {
        if (declaration.id.type === 'Identifier') {
          ...
          if (
            HTTP_METHODS.includes(declaration.id.name as Method) &&
            !handlerStacks.has(declaration.id.name)
          ) {
            invalidMethods.add(declaration.id.name as Method);
          }
          if (HTTP_METHODS.includes(declaration.id.name as Method)) {
            methodStackNames.set(
              declaration.id.name as Method,
              declaration.id.name
            );
          }
        }
  ...
  return {
    arities: Object.fromEntries(
      methods.map((method) => [
        method,
        handlerStacks.get(methodStackNames.get(method) ?? method) ?? null,
      ])
    ),
    defaultIsHono,
 ...
 */
function handlerStackArity(
  value: unknown,
  factories: Set<string>,
  stacks: Map<string, number | null>
  ...
  if (node.type === 'CallExpression') {
    ...
    return callee?.type === 'Identifier' &&
      ...
      ? countHandlers(arguments_)
      : undefined;
  }
  if (isTransparentExpression(node)) {
    return handlerStackArity(node.expression, factories, stacks);
  }

  return undefined;
}

function countHandlers(elements: unknown[]): number | null {
