...
async function withGenerationLock<T>(
  ...
  // Kept inside the generated directory so a single `.shinro/` ignore rule
  // covers it, and so watchers that already skip generated output are not woken
  // by Shinro taking its own lock.
  ...
  while (true) {
    ...
    try {
      ...
      if (Date.now() - startedAt >= GENERATION_LOCK_TIMEOUT_MS) {
        throw new Error(
          `[shinro] Timed out waiting for another process to finish generating ${outputDirectory}. If no typegen process is running, remove ${lockFile} and try again.`
        );
      }
...
// still recognised as Shinro's. A file without one belongs to the user.
