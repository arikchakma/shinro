...
import type { ShinroOptions } from './plugin.ts';
...
export async function createSources(
  ...
  try {
    ...
    if ((error as NodeJS.ErrnoException).code === 'ENOENT') {
      throw new Error(
        `[shinro] App module not found: ${appFile}\nCreate it with a default defineApp() export or configure shinro({ app: "path/to/app.ts" }).`,
        { cause: error }
      );
    }
    throw error;
  ...
  if (!appAnalysis.mountsRoutes && routes.length > 0) {
    config.logger.warn(
      `[shinro] ${toProjectPath(config.root, appFile)} never mounts ${ROUTES_ID}, so ${
        routes.length
