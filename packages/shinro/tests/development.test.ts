...
test('the watcher picks up a brand-new route file', async () => {
  await withProject('watch-new-route', async (project) => {
    ...
    try {
      // Node owns the process: a signal handler here would owe the user SIGTERM
      // semantics.
      ...
      // Nothing imports the new file yet, so only a directory watcher sees it.
      await waitFor(async () =>
...
test('the preload never keeps a drained server process alive', async () => {
  ...
  try {
    ...
    // `node --watch` would stall on "waiting for graceful termination".
    await expect(
