...
test('ancestor directory middleware stack root-to-leaf', async () => {
  ...
  const root = await mkdtemp(`${packageRoot}/.shinro-middleware-hierarchy-`);
...
test('directory middleware surrounds a default sub-router mount exactly once', async () => {
  ...
  const root = await mkdtemp(
    `${packageRoot}/.shinro-subrouter-middleware-runtime-`
  );
...
test('conflicting file routes fail configuration with both source files', async () => {
  ...
  await expect(
    resolveConfig({ configFile: false, plugins: [shinro()], root }, 'serve')
  ).rejects.toThrow(
...
test('equivalent dynamic route shapes conflict even when parameter names differ', async () => {
  const root = await mkdtemp(`${tmpdir()}/shinro-dynamic-shape-conflict-`);
  ...
  try {
    await expect(
      ...
      /\[shinro\][\s\S]*\/users\/:id[\s\S]*(?:\$id\.ts[\s\S]*\$slug\.ts|\$slug\.ts[\s\S]*\$id\.ts)/i
    );
  } finally {
    await rm(root, { recursive: true });
  }
...
test('a default sub-router reports ownership of its descendant namespace', async () => {
  const root = await mkdtemp(
    `${tmpdir()}/shinro-subrouter-namespace-conflict-`
  );
  await mkdir(`${root}/src/routes/admin`, { recursive: true });
  await writeFile(`${root}/src/app.ts`, temporaryAppSource);
  await writeFile(
  ...
  try {
    await expect(
      ...
      /\[shinro\][\s\S]*\/admin[\s\S]*admin\.ts[\s\S]*admin\/stats\.ts[\s\S]*owns[\s\S]*namespace/i
    );
  } finally {
    await rm(root, { recursive: true });
  }
...
test('a default sub-router namespace rejects dynamically matching descendants', async () => {
  const root = await mkdtemp(`${tmpdir()}/shinro-dynamic-namespace-conflict-`);
  ...
  try {
    await expect(
      ...
      /\[shinro\][\s\S]*namespace conflict[\s\S]*\/admin[\s\S]*(?:admin\.ts[\s\S]*\$section\/stats\.ts|\$section\/stats\.ts[\s\S]*admin\.ts)/i
    );
  } finally {
    await rm(root, { recursive: true });
  }
...
test('a package that exposes generated RPC warns when its client export is missing', async () => {
  const root = await mkdtemp(`${tmpdir()}/shinro-client-export-warning-`);
  ...
  try {
    await resolveConfig(
      { configFile: false, customLogger: logger, plugins: [shinro()], root },
      'serve'
    );

    expect(warnings.join('\n')).toMatch(
      /\[shinro\][\s\S]*package\.json[\s\S]*generated client[\s\S]*\.\/client[\s\S]*\.shinro\/client\.ts/i
    );
  } finally {
    await rm(root, { recursive: true });
  }
...
test('generation writes a project-relative normalized manifest', async () => {
  const manifest = JSON.parse(
    await readFile(new URL('../.shinro/manifest.json', import.meta.url), 'utf8')
  ) as {
    routes: Array<{
      file: string;
      kind: string;
...
test('a route companion exposes its contract through Route.Handler', async () => {
  ...
  try {
    ...
    expect(source).toMatch(
      /export namespace Route \{[\s\S]*export type Handler = ShinroRoute<\{[\s\S]*path: "\/users\/:id"/
    );
...
test('route companions are generated correctly when the project path contains spaces', async () => {
  ...
  try {
    ...
    await expect(
      readFile(`${root}/.shinro/types/src/routes/+types/health.d.ts`, 'utf8')
...
test('unchanged generated files retain their modification time', async () => {
  ...
  const rpcFile = new URL('../.shinro/rpc.ts', import.meta.url);
...
test('a generation write failure leaves the previous RPC contract intact', async () => {
  const root = await mkdtemp(`${tmpdir()}/shinro-generation-transaction-`);
  await mkdir(`${root}/src/routes`, { recursive: true });
  await writeFile(`${root}/src/app.ts`, temporaryAppSource);
  await writeFile(
    `${root}/src/routes/health.ts`,
  ...
  try {
    ...
    const previousManifest = await readFile(
      `${root}/.shinro/manifest.json`,
      'utf8'
    );
    const previousRpc = await readFile(`${root}/.shinro/rpc.ts`, 'utf8');

    await writeFile(
      `${root}/src/routes/api.ts`,
      'export const GET = [(c: any) => c.json({ api: true })] as const;\n'
    );
    await mkdir(`${root}/.shinro/types/src/routes/+types/api.d.ts`, {
      recursive: true,
    });
    ...
    await expect(
      readFile(`${root}/.shinro/manifest.json`, 'utf8')
    ).resolves.toBe(previousManifest);
    await expect(readFile(`${root}/.shinro/rpc.ts`, 'utf8')).resolves.toBe(
      previousRpc
    );
  } finally {
    await rm(root, { recursive: true });
...
test('generated artifacts identify their format and warn against editing', async () => {
  const rpc = await readFile(
    new URL('../.shinro/rpc.ts', import.meta.url),
    'utf8'
  );
  const manifest = await readFile(
    new URL('../.shinro/manifest.json', import.meta.url),
    'utf8'
  );

  expect(rpc).toMatch(/^\/\/ Generated by Shinro \(format 2\)\. Do not edit\./);
  expect(manifest).toMatch(
    /^\{\n  "_notice": "Generated by Shinro \(format 2\)\. Do not edit\."/
  );
});
...
test('basePath prefixes normalized runtime and RPC routes', async () => {
  ...
  await resolveConfig(
    { configFile: false, plugins: [shinro({ basePath: '/v1/' })], root },
    'serve'
  );
  const manifest = JSON.parse(
    await readFile(
      new URL('./fixtures/basepath/.shinro/manifest.json', import.meta.url),
      'utf8'
    )
...
test('rpc.outDir refuses a directory holding files Shinro did not generate', async () => {
  const root = await mkdtemp(`${tmpdir()}/shinro-outdir-guard-`);
...
test('a partially generated directory is still recognised as Shinro output', async () => {
  const root = await mkdtemp(`${tmpdir()}/shinro-partial-outdir-`);

  await mkdir(`${root}/src/routes`, { recursive: true });
  await mkdir(`${root}/.shinro`, { recursive: true });
  await writeFile(`${root}/src/app.ts`, temporaryAppSource);
  await writeFile(
    `${root}/src/routes/health.ts`,
    'export const GET = [(c: any) => c.json({ ok: true })] as const;\n'
  ...
  await writeFile(`${root}/.shinro/shinro.d.ts`, '');
  await writeFile(`${root}/.shinro/routes.ts`, '');
...
test('ALL serves every verb and yields to an explicit method export', async () => {
  ...
  const root = await mkdtemp(
    fileURLToPath(new URL('../.shinro-all-method-', import.meta.url))
  );
...
test('a lock left by a dead process is reclaimed instead of timing out', async () => {
  const root = await mkdtemp(`${tmpdir()}/shinro-stale-lock-`);
  ...
  await mkdir(`${root}/.shinro`, { recursive: true });
  await writeFile(`${root}/.shinro/.lock`, `${dead.pid}\n`);
  ...
  try {
    ...
    await expect(
      readFile(`${root}/.shinro/.lock`, 'utf8')
...
test('the development child leaves generation and diagnostics to its parent', async () => {
  const root = await mkdtemp(`${tmpdir()}/shinro-dev-child-`);
  const warnings: string[] = [];
  const logger = createLogger('silent');
  logger.warn = (message) => {
    warnings.push(message);
  ...
  process.env.SHINRO_DEV_CHILD = '1';
  try {
    await resolveConfig(
      { configFile: false, customLogger: logger, plugins: [shinro()], root },
      'serve'
    ...
    delete process.env.SHINRO_DEV_CHILD;
    await rm(root, { recursive: true });
  }
});
...
test('basePath collapses repeated separators instead of emptying a segment', async () => {
  ...
  await resolveConfig(
    {
      configFile: false,
      plugins: [shinro({ basePath: '//v1//api//' })],
      root,
    },
    'serve'
  );
...
test('an invalid basePath fails with the Shinro option name', async () => {
  const root = fileURLToPath(new URL('./fixtures/basepath', import.meta.url));
  ...
  await expect(
    resolveConfig(
      {
        configFile: false,
        plugins: [shinro({ basePath: 'api' as '/api' })],
        root,
      },
      'serve'
    )
  ).rejects.toThrow(/\[shinro\][\s\S]*basePath[\s\S]*start[\s\S]*\//i);
});
...
test('reserved basenames and directories are excluded from route discovery', async () => {
  const packageRoot = fileURLToPath(new URL('..', import.meta.url));
  const root = await mkdtemp(`${packageRoot}/.shinro-ignored-routes-`);
  const ignoredFiles = [
...
test('ignoredRouteFiles excludes route-relative globs', async () => {
  ...
  const server = await createServer({
    configFile: false,
    customLogger: createLogger('silent'),
    plugins: [shinro({ ignoredRouteFiles: ['internal/**'] })],
    root,
    server: { middlewareMode: true },
  });
...
test('rpc.outDir relocates all generated artifacts', async () => {
  ...
  await resolveConfig(
    {
      configFile: false,
      plugins: [shinro({ rpc: { outDir: '.generated' } })],
      root,
    },
    'serve'
  );
...
test('rpc.outDir cannot target the project root', async () => {
  const root = await mkdtemp(`${tmpdir()}/shinro-root-output-`);
  await mkdir(`${root}/src/routes`, { recursive: true });
  await writeFile(`${root}/src/app.ts`, temporaryAppSource);
  await writeFile(
    `${root}/src/routes/index.ts`,
  ...
  try {
    await expect(
      resolveConfig(
        {
          configFile: false,
          plugins: [shinro({ rpc: { outDir: '.' } })],
          root,
        },
        'serve'
      )
    ).rejects.toThrow(
      /\[shinro\][\s\S]*rpc\.outDir[\s\S]*project root[\s\S]*(?:generated|directory)/i
    );
  } finally {
    await rm(root, { recursive: true });
  }
...
test('one TypeScript project cannot install multiple Shinro applications', async () => {
  const root = await mkdtemp(`${tmpdir()}/shinro-multiple-apps-`);
  ...
  try {
    await expect(
      resolveConfig(
        {
          configFile: false,
          plugins: [shinro(), shinro()],
          root,
        },
        'serve'
      )
    ).rejects.toThrow(
      /\[shinro\][\s\S]*(?:one|multiple)[\s\S]*(?:application|plugin)[\s\S]*TypeScript project/i
    );
  } finally {
    await rm(root, { recursive: true });
  }
...
test('adding a route during development regenerates every route-derived artifact', async () => {
  const root = await mkdtemp(
    fileURLToPath(new URL('../.shinro-route-add-', import.meta.url))
  );
  ...
  try {
    ...
    await expect(
      server.transformRequest('shinro/routes')
...
test('routes register in static, dynamic, then catch-all priority', async () => {
  const root = await mkdtemp(`${tmpdir()}/shinro-route-order-`);
...
test('a catch-all segment before the end of a route fails with its source file', async () => {
  const root = await mkdtemp(`${tmpdir()}/shinro-invalid-catch-all-`);
...
test('an invalid dynamic parameter name fails with its source file and parameter', async () => {
  const root = await mkdtemp(`${tmpdir()}/shinro-invalid-parameter-`);
...
test('duplicate dynamic parameter names in one route fail before generation', async () => {
  const root = await mkdtemp(`${tmpdir()}/shinro-duplicate-route-parameter-`);
  ...
  try {
    await expect(
      ...
      /\[shinro\][\s\S]*teams\/\$id\/members\/\$id\.ts[\s\S]*duplicate[\s\S]*parameter[\s\S]*"id"/i
    );
  } finally {
    await rm(root, { recursive: true });
  }
...
test('a route cannot mix a default sub-router with named method exports', async () => {
  const root = await mkdtemp(`${tmpdir()}/shinro-mixed-route-`);
  const routesDirectory = `${root}/src/routes`;
...
test('a named method exported through an export list is discovered', async () => {
  const root = await mkdtemp(`${tmpdir()}/shinro-export-list-`);
  const routesDirectory = `${root}/src/routes`;
...
test('directory middleware receives an optional generated companion type', async () => {
  ...
  try {
    ...
    expect(source).toMatch(
      /export namespace Route \{[\s\S]*export type Middleware = ShinroMiddleware<\{[\s\S]*path: "\/api"/
    );
...
test('a production build fails when it emits more than one JavaScript chunk', async () => {
  const packageRoot = fileURLToPath(new URL('..', import.meta.url));
  const root = await mkdtemp(`${packageRoot}/.shinro-split-build-`);
  ...
  try {
    await expect(
      build({
        configFile: false,
        logLevel: 'silent',
        plugins: [shinro({ build: { unbundle: false } })],
        root,
      })
    ).rejects.toThrow(
      /shinro[\s\S]*multiple javascript chunks[\s\S]*single entry/i
    );
  } finally {
    await rm(root, { recursive: true });
  }
...
test('the default build keeps the server entry but preserves the source module tree', async () => {
  const packageRoot = fileURLToPath(new URL('..', import.meta.url));
  const root = await mkdtemp(`${packageRoot}/.shinro-unbundle-`);
...
test('a production build warns when it emits an external runtime asset', async () => {
  const packageRoot = fileURLToPath(new URL('..', import.meta.url));
  const root = await mkdtemp(`${packageRoot}/.shinro-external-asset-`);
  const warnings: string[] = [];
  const logger = createLogger('silent');
  logger.warn = (message) => {
    warnings.push(message);
  ...
  try {
    ...
    expect(warnings.join('\n')).toMatch(
      /\[shinro\][\s\S]*external runtime asset[\s\S]*one-entry/i
    );
  } finally {
    await rm(root, { recursive: true });
  }
...
test('a production build rejects an unexpected server entry filename', async () => {
  const packageRoot = fileURLToPath(new URL('..', import.meta.url));
  const root = await mkdtemp(`${packageRoot}/.shinro-entry-filename-`);
  ...
  try {
    await expect(
      build({
        ...
        logLevel: 'silent',
        plugins: [
          shinro({ build: { unbundle: false } }),
          {
            name: 'override-shinro-entry-filename',
            config: () => ({
              build: {
                rolldownOptions: {
      ...
      /\[shinro\][\s\S]*unexpected\.mjs[\s\S]*server\.mjs[\s\S]*entry filename/i
    );
  } finally {
    await rm(root, { recursive: true });
  }
...
test('rpc.enabled false keeps routing typegen but omits RPC client artifacts', async () => {
  const root = await mkdtemp(`${tmpdir()}/shinro-rpc-disabled-`);
  ...
  try {
    await resolveConfig(
      {
        configFile: false,
        plugins: [shinro({ rpc: { enabled: false } })],
        root,
      },
      'serve'
    );
    ...
    await expect(
      readFile(`${root}/.shinro/rpc.ts`, 'utf8')
    ...
    // declaration that gives `shinro/routes` its type, are both still there.
    await expect(
      readFile(`${root}/.shinro/routes.ts`, 'utf8')
    ).resolves.toContain('.get("/health"');
    const modules = await readFile(`${root}/.shinro/shinro.d.ts`, 'utf8');
    expect(modules).toContain('declare module "shinro/routes"');
    expect(modules).not.toContain('declare module "shinro/client"');
  } finally {
    await rm(root, { recursive: true });
  }
});
...
test('an app that never mounts the generated router warns with the route count', async () => {
  const root = await mkdtemp(`${tmpdir()}/shinro-missing-mount-`);
  const warnings: string[] = [];
  const logger = createLogger('silent');
  logger.warn = (message) => {
    warnings.push(message);
...
test('an app that mounts the generated router is not warned about', async () => {
  const root = await mkdtemp(`${tmpdir()}/shinro-mounted-`);
  const warnings: string[] = [];
  const logger = createLogger('silent');
  logger.warn = (message) => {
    warnings.push(message);
...
test('an app with no file routes is not asked to mount anything', async () => {
  const root = await mkdtemp(`${tmpdir()}/shinro-no-routes-no-mount-`);
  const warnings: string[] = [];
  const logger = createLogger('silent');
  logger.warn = (message) => {
    warnings.push(message);
...
test('middleware chained after the mount warns about registration order', async () => {
  const root = await mkdtemp(`${tmpdir()}/shinro-mount-order-chained-`);
  const warnings: string[] = [];
  const logger = createLogger('silent');
  logger.warn = (message) => {
    warnings.push(message);
  ...
  try {
    ...
    expect(warnings.join('\n')).toMatch(
      /\[shinro\][\s\S]*src\/app\.ts[\s\S]*registration order[\s\S]*file route/i
    );
  } finally {
    await rm(root, { recursive: true });
  }
...
test('a middleware statement after the mount warns about registration order', async () => {
  const root = await mkdtemp(`${tmpdir()}/shinro-mount-order-statement-`);
  const warnings: string[] = [];
  const logger = createLogger('silent');
  logger.warn = (message) => {
    warnings.push(message);
...
test('middleware registered before the mount is not warned about', async () => {
  const root = await mkdtemp(`${tmpdir()}/shinro-mount-order-correct-`);
  const warnings: string[] = [];
  const logger = createLogger('silent');
  logger.warn = (message) => {
    warnings.push(message);
...
test('generating over an older project removes its entry artifacts', async () => {
  const root = await mkdtemp(`${tmpdir()}/shinro-legacy-cleanup-`);
  ...
  // `.shinro` is gitignored, so every existing project starts generation with
  // the previous format's files still on disk. A stale `modules.d.ts` is not
  // inert: it declares `shinro/entry` against an `entry.ts` that still imports
  // the app and registers an outdated route table.
  await writeFile(
    `${root}/.shinro/entry.ts`,
    '// Generated by Shinro (format 1). Do not edit.\nexport default {};\n'
  );
  await writeFile(
    `${root}/.shinro/modules.d.ts`,
    '// Generated by Shinro (format 1). Do not edit.\ndeclare module "shinro/entry" {}\n'
  );
  ...
  try {
    ...
    for (const file of ['entry.ts', 'modules.d.ts']) {
      await expect(
        readFile(`${root}/.shinro/${file}`, 'utf8')
      ).rejects.toMatchObject({ code: 'ENOENT' });
    }
    await expect(
      readFile(`${root}/.shinro/routes.ts`, 'utf8')
...
test('a hand-written file the previous format never generated is left alone', async () => {
  const root = await mkdtemp(`${tmpdir()}/shinro-legacy-handwritten-`);

  await mkdir(`${root}/src/routes`, { recursive: true });
  await mkdir(`${root}/.shinro`, { recursive: true });
  await writeFile(`${root}/src/app.ts`, temporaryAppSource);
  // No generated notice, so removal must refuse to touch it even though the
  // name is one Shinro used to own.
  await writeFile(`${root}/.shinro/entry.ts`, 'export const mine = true;\n');
  ...
  try {
    ...
    await expect(
      readFile(`${root}/.shinro/entry.ts`, 'utf8')
...
test('every specifier the generated declarations name is importable', async () => {
  const root = await mkdtemp(
    fileURLToPath(new URL('../.shinro-specifiers-', import.meta.url))
  );
  ...
  try {
    const declarations = await readFile(`${root}/.shinro/shinro.d.ts`, 'utf8');
    const declared = [
      ...declarations.matchAll(/declare module "(shinro\/[^"]+)"/g),
    ].map((match) => match[1]);

    expect(declared).toEqual(['shinro/routes', 'shinro/client', 'shinro/rpc']);
    ...
    const routesModule = (await runner.import('shinro/routes')) as {
      routes: unknown;
    };
...
test('a missing routes directory reports the resolved path and configuration option', async () => {
  const root = await mkdtemp(`${tmpdir()}/shinro-missing-routes-`);
  ...
  try {
    await expect(
      ...
      /\[shinro\][\s\S]*routes directory[\s\S]*src\/routes[\s\S]*routes:/i
    );
  } finally {
    await rm(root, { recursive: true });
  }
...
test('a missing app module reports the resolved path and configuration option', async () => {
  const root = await mkdtemp(`${tmpdir()}/shinro-missing-app-`);
  ...
  try {
    await expect(
      ...
      /\[shinro\][\s\S]*app module[\s\S]*src\/app\.ts[\s\S]*app:/i
    );
  } finally {
    await rm(root, { recursive: true });
  }
...
test('the app module must default-export an instance created by defineApp', async () => {
  const root = await mkdtemp(`${tmpdir()}/shinro-invalid-app-`);
  ...
  try {
    await expect(
      ...
      /\[shinro\][\s\S]*src\/app\.ts[\s\S]*default[\s\S]*defineApp\(\)/i
    );
  } finally {
    await rm(root, { recursive: true });
  }
...
test('an app syntax error reports the configured source file', async () => {
  const root = await mkdtemp(`${tmpdir()}/shinro-app-syntax-error-`);
  await mkdir(`${root}/src/routes`, { recursive: true });
  ...
  try {
    await expect(
      ...
      /\[shinro\][\s\S]*(?:parse|syntax)[\s\S]*src\/app\.ts|src\/app\.ts[\s\S]*(?:parse|syntax)/i
    );
  } finally {
    await rm(root, { recursive: true });
  }
...
test('the app module may retain Hono schema through a chained defineApp instance', async () => {
  const root = await mkdtemp(`${tmpdir()}/shinro-chained-app-`);
  await mkdir(`${root}/src/routes`, { recursive: true });
  await writeFile(
    `${root}/src/app.ts`,
    [
...
test('the app module may default-export a plain new Hono() instance', async () => {
  const root = await mkdtemp(`${tmpdir()}/shinro-plain-hono-app-`);
  await mkdir(`${root}/src/routes`, { recursive: true });
  await writeFile(
    `${root}/src/app.ts`,
...
test('a chained plain Hono app is accepted as the application root', async () => {
  const root = await mkdtemp(`${tmpdir()}/shinro-plain-hono-chained-`);
  await mkdir(`${root}/src/routes`, { recursive: true });
  await writeFile(
    `${root}/src/app.ts`,
    [
...
test('the app instance may use a local default export list', async () => {
  const root = await mkdtemp(`${tmpdir()}/shinro-app-export-list-`);
  await mkdir(`${root}/src/routes`, { recursive: true });
  await writeFile(
    `${root}/src/app.ts`,
    [
...
test('a default route export must be a Hono sub-router', async () => {
  const root = await mkdtemp(`${tmpdir()}/shinro-invalid-sub-router-`);
  ...
  try {
    await expect(
      ...
      /\[shinro\][\s\S]*src\/routes\/admin\.ts[\s\S]*default export[\s\S]*Hono sub-router/i
    );
  } finally {
    await rm(root, { recursive: true });
  }
...
test('a chained Hono sub-router may use a local default export list', async () => {
  const root = await mkdtemp(`${tmpdir()}/shinro-subrouter-export-list-`);
...
test('a default sub-router rejects unchained route mutations', async () => {
  const root = await mkdtemp(`${tmpdir()}/shinro-unchained-subrouter-`);
  ...
  try {
    await expect(
      ...
      /\[shinro\][\s\S]*src\/routes\/admin\.ts[\s\S]*(?:chain|chained)[\s\S]*(?:RPC|schema)/i
    );
  } finally {
    await rm(root, { recursive: true });
  }
...
test('a default sub-router rejects unchained middleware mutations', async () => {
  const root = await mkdtemp(
    `${tmpdir()}/shinro-unchained-subrouter-middleware-`
  );
...
test('a named method export must be a handler tuple', async () => {
  const root = await mkdtemp(`${tmpdir()}/shinro-invalid-handler-export-`);
  ...
  try {
    await expect(
      ...
      /\[shinro\][\s\S]*src\/routes\/health\.ts[\s\S]*GET[\s\S]*defineHandler[\s\S]*tuple/i
    );
  } finally {
    await rm(root, { recursive: true });
  }
...
test('a supported method function declaration is rejected instead of silently ignored', async () => {
  const root = await mkdtemp(`${tmpdir()}/shinro-method-function-export-`);
...
test('an external method re-export is rejected when its handler tuple cannot be proven', async () => {
  const root = await mkdtemp(`${tmpdir()}/shinro-method-reexport-`);
...
test('a named method cannot export an empty handler tuple', async () => {
  const root = await mkdtemp(`${tmpdir()}/shinro-empty-handler-export-`);
...
test('a named method may come from a project wrapper or a shared tuple', async () => {
  const root = await mkdtemp(`${tmpdir()}/shinro-wrapped-handler-`);
  ...
  try {
    ...
    const router = await readFile(`${root}/.shinro/routes.ts`, 'utf8');
...
test('a named method tuple rejects values that cannot be handlers', async () => {
  const root = await mkdtemp(`${tmpdir()}/shinro-invalid-handler-value-`);
  ...
  try {
    await expect(
      ...
      /\[shinro\][\s\S]*src\/routes\/health\.js[\s\S]*GET[\s\S]*(?:handler|tuple)/i
    );
  } finally {
    await rm(root, { recursive: true });
  }
...
test('a route syntax error reports the source file with Shinro context', async () => {
  const root = await mkdtemp(`${tmpdir()}/shinro-route-syntax-error-`);
  ...
  try {
    await expect(
      ...
      /\[shinro\][\s\S]*(?:parse|syntax)[\s\S]*src\/routes\/health\.ts|src\/routes\/health\.ts[\s\S]*(?:parse|syntax)/i
    );
  } finally {
    await rm(root, { recursive: true });
  }
...
test('a route file with no supported method exports is ignored with a warning', async () => {
  const root = await mkdtemp(`${tmpdir()}/shinro-empty-route-`);
  const warnings: string[] = [];
  const logger = createLogger('silent');
  logger.warn = (message) => {
    warnings.push(message);
  ...
  try {
    ...
    expect(warnings.join('\n')).toMatch(
      /\[shinro\][\s\S]*src\/routes\/helpers\.ts[\s\S]*no supported method[\s\S]*GET/i
    );
  } finally {
    await rm(root, { recursive: true });
  }
...
test('directory middleware around a default sub-router warns about its RPC boundary', async () => {
  const root = await mkdtemp(
    `${tmpdir()}/shinro-subrouter-middleware-warning-`
  );
  const warnings: string[] = [];
  const logger = createLogger('silent');
  logger.warn = (message) => {
  ...
  try {
    ...
    expect(warnings.join('\n')).toMatch(
      /\[shinro\][\s\S]*src\/routes\/admin\/index\.ts[\s\S]*directory middleware[\s\S]*RPC[\s\S]*response/i
    );
  } finally {
    await rm(root, { recursive: true });
  }
...
test('base-app early-response middleware warns about file-route RPC contracts', async () => {
  const root = await mkdtemp(`${tmpdir()}/shinro-base-middleware-warning-`);
  const warnings: string[] = [];
  const logger = createLogger('silent');
  logger.warn = (message) => {
    warnings.push(message);
  ...
  try {
    ...
    expect(warnings.join('\n')).toMatch(
      /\[shinro\][\s\S]*src\/app\.ts[\s\S]*base-app middleware[\s\S]*RPC[\s\S]*response/i
    );
  } finally {
    await rm(root, { recursive: true });
  }
...
test('a provable parameter schema and filename mismatch emits a warning', async () => {
  const root = await mkdtemp(`${tmpdir()}/shinro-param-schema-warning-`);
  const warnings: string[] = [];
  const logger = createLogger('silent');
  logger.warn = (message) => {
    warnings.push(message);
  ...
  try {
    ...
    expect(warnings.join('\n')).toMatch(
      /\[shinro\][\s\S]*src\/routes\/users\/\$id\.ts[\s\S]*parameter schema[\s\S]*userId[\s\S]*id/i
    );
  } finally {
    await rm(root, { recursive: true });
  }
...
test('a parameter schema mismatch is detected for non-zod Hono validators', async () => {
  ...
  for (const [module, factory] of validators) {
    const root = await mkdtemp(`${tmpdir()}/shinro-param-schema-${factory}-`);
    const warnings: string[] = [];
    const logger = createLogger('silent');
    logger.warn = (message) => {
      warnings.push(message);
...
test('a parameter schema mismatch is detected through a method export list', async () => {
  const root = await mkdtemp(
    `${tmpdir()}/shinro-exported-param-schema-warning-`
  );
  const warnings: string[] = [];
  const logger = createLogger('silent');
  logger.warn = (message) => {
...
test('a parameter schema mismatch is detected through a local schema variable', async () => {
  const root = await mkdtemp(`${tmpdir()}/shinro-local-param-schema-warning-`);
  const warnings: string[] = [];
  const logger = createLogger('silent');
  logger.warn = (message) => {
    warnings.push(message);
...
test('a production build with no server entry fails before bundling with guidance', async () => {
  const packageRoot = fileURLToPath(new URL('..', import.meta.url));
  const root = await mkdtemp(`${packageRoot}/.shinro-missing-entry-`);
...
test('an incompatible TypeScript config receives a copy-pasteable correction', async () => {
  const root = await mkdtemp(`${tmpdir()}/shinro-invalid-tsconfig-`);
  const warnings: string[] = [];
  const logger = createLogger('silent');
  logger.warn = (message) => {
    warnings.push(message);
  ...
  try {
    ...
    expect(warnings.join('\n')).toMatch(
      /\[shinro\][\s\S]*tsconfig\.json[\s\S]*"allowImportingTsExtensions": true[\s\S]*"strict": true[\s\S]*"module": "ESNext"[\s\S]*"moduleResolution": "Bundler"[\s\S]*"noEmit": true[\s\S]*"rootDirs"[\s\S]*\.shinro\/types[\s\S]*\.shinro\/\*\*\/\*\.ts/i
    );
  } finally {
    await rm(root, { recursive: true });
  }
...
test('a custom rpc.outDir is not told to extend the shipped base config', async () => {
  const root = await mkdtemp(`${tmpdir()}/shinro-custom-outdir-tsconfig-`);
  const warnings: string[] = [];
  const logger = createLogger('silent');
  logger.warn = (message) => {
    warnings.push(message);
  ...
  try {
    ...
    expect(warning).not.toContain('"extends": "shinro/tsconfig"');
  } finally {
    await rm(root, { recursive: true });
  }
});
...
test('a TypeScript config extending a list of bases is understood', async () => {
  const root = await mkdtemp(`${tmpdir()}/shinro-extends-list-`);
  const warnings: string[] = [];
  const logger = createLogger('silent');
  logger.warn = (message) => {
    warnings.push(message);
  ...
  await writeFile(
    ...
    JSON.stringify({
      ...
      include: ['src', '.shinro/**/*.d.ts', '.shinro/**/*.ts'],
    })
  );
  await writeFile(
    `${root}/tsconfig.json`,
...
test('a missing TypeScript config receives the generated-types configuration', async () => {
  const root = await mkdtemp(`${tmpdir()}/shinro-missing-tsconfig-`);
  const warnings: string[] = [];
  const logger = createLogger('silent');
  logger.warn = (message) => {
    warnings.push(message);
  ...
  try {
    ...
    expect(warnings.join('\n')).toMatch(
      /\[shinro\][\s\S]*tsconfig\.json[\s\S]*missing[\s\S]*"allowImportingTsExtensions": true[\s\S]*"strict": true[\s\S]*"noEmit": true[\s\S]*"rootDirs"[\s\S]*\.shinro\/types[\s\S]*include/i
    );
  } finally {
    await rm(root, { recursive: true });
  }
...
test('TypeScript settings inherited from a relative config are accepted', async () => {
  const root = await mkdtemp(`${tmpdir()}/shinro-extended-tsconfig-`);
  const warnings: string[] = [];
  const logger = createLogger('silent');
  logger.warn = (message) => {
    warnings.push(message);
...
test('reading a tsconfig keeps punctuation inside string values', async () => {
  const root = await mkdtemp(`${tmpdir()}/shinro-jsonc-tsconfig-`);
  const warnings: string[] = [];
  const logger = createLogger('silent');
  logger.warn = (message) => {
    warnings.push(message);
...
test('TypeScript settings inherited from a package config are accepted', async () => {
  const root = await mkdtemp(`${tmpdir()}/shinro-package-tsconfig-`);
  const warnings: string[] = [];
  const logger = createLogger('silent');
  logger.warn = (message) => {
    warnings.push(message);
...
test.each(['emitDeclarationOnly', 'rewriteRelativeImportExtensions'] as const)(
  ...
  async (emissionOption) => {
    const root = await mkdtemp(`${tmpdir()}/shinro-ts-extension-emission-`);
    const warnings: string[] = [];
    const logger = createLogger('silent');
    logger.warn = (message) => {
      warnings.push(message);
...
test('_middleware.js is discovered for JavaScript route projects', async () => {
  const root = await mkdtemp(`${tmpdir()}/shinro-js-middleware-`);
...
test('directory middleware must default-export a middleware tuple', async () => {
  const root = await mkdtemp(`${tmpdir()}/shinro-invalid-middleware-`);
  ...
  try {
    await expect(
      ...
      /\[shinro\][\s\S]*src\/routes\/api\/_middleware\.ts[\s\S]*default[\s\S]*defineMiddleware[\s\S]*tuple/i
    );
  } finally {
    await rm(root, { recursive: true });
  }
...
test('directory middleware cannot export an empty middleware tuple', async () => {
  const root = await mkdtemp(`${tmpdir()}/shinro-empty-middleware-`);
  await mkdir(`${root}/src/routes/api`, { recursive: true });
  await writeFile(`${root}/src/app.ts`, temporaryAppSource);
  await writeFile(
    `${root}/src/routes/api/_middleware.ts`,
  ...
  try {
    await expect(
      ...
      /\[shinro\][\s\S]*src\/routes\/api\/_middleware\.ts[\s\S]*(?:one|non-empty)[\s\S]*middleware/i
    );
  } finally {
    await rm(root, { recursive: true });
  }
...
test('a group directory wraps its routes in middleware without entering the URL', async () => {
  const packageRoot = fileURLToPath(new URL('..', import.meta.url));
  const root = await mkdtemp(`${packageRoot}/.shinro-route-group-`);
  const helper = JSON.stringify(
    fileURLToPath(new URL('../src/app.ts', import.meta.url))
  );
...
test('a group directory keeps its middleware in the manifest but not in the path', async () => {
  const root = await mkdtemp(`${tmpdir()}/shinro-route-group-manifest-`);
  const helper = JSON.stringify(
    fileURLToPath(new URL('../src/app.ts', import.meta.url))
  );
...
test('group directories and escaped segments derive their URLs', async () => {
  const root = await mkdtemp(`${tmpdir()}/shinro-segment-derivation-`);
  const expected = {
    '(a)/orders.ts': '/orders',
...
test('malformed groups and unserviceable segments fail with their source file', async () => {
  ...
  for (const { file, pattern } of cases) {
    const root = await mkdtemp(`${tmpdir()}/shinro-invalid-segment-`);
    const directory = file.includes('/')
      ? `/${file.slice(0, file.lastIndexOf('/'))}`
      : '';
...
test('a malformed group in directory middleware names the middleware file', async () => {
  const root = await mkdtemp(`${tmpdir()}/shinro-group-middleware-error-`);
  const helper = JSON.stringify(
    fileURLToPath(new URL('../src/app.ts', import.meta.url))
  );
...
test('routes that collapse onto one URL through a group explain the collapse', async () => {
  ...
  for (const files of layouts) {
    const root = await mkdtemp(`${tmpdir()}/shinro-group-conflict-`);
...
test('a production build emits a grouped route under its on-disk path', async () => {
  const packageRoot = fileURLToPath(new URL('..', import.meta.url));
  const root = await mkdtemp(`${packageRoot}/.shinro-group-build-`);
