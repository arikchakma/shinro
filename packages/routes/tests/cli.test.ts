  ...
  try {
    ...
    for (const file of [
      ...
      await expect(readFile(`${root}/${file}`, 'utf8')).resolves.toContain(
        'Generated by Daroyan (format 2)'
      );
    }
    for (const file of ['.daroyan/entry.ts', '.daroyan/modules.d.ts']) {
    ...
    ).resolves.toContain('from "./client.ts";');
    await expect(
    ...
    ).resolves.toContain('declare module "daroyan/routes"');
