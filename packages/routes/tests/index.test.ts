...
test('a partially generated directory is still recognised as Daroyan output', async () => {
  ...
  await writeFile(`${root}/.daroyan/entry.ts`, '');
...
test('output from an earlier version is adopted and cleaned up', async () => {
  ...
  try {
    ...
    // The renamed declaration replaces it; a stale `entry.d.ts` would be
    // shadowed by `entry.ts` and silently stop declaring `daroyan/entry`.
    await expect(
      readFile(`${root}/.daroyan/modules.d.ts`, 'utf8')
    ).resolves.toContain('declare module "daroyan/entry"');
    await expect(
      readFile(`${root}/.daroyan/entry.d.ts`, 'utf8')
