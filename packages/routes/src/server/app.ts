...
export async function validateAppModule(file: string): Promise<AppModuleAnalysis> {
  ...
  for (const statement of ast.body) {
    ...
    for (const variable of declaration.declarations) {
      if (variable.id.type === "Identifier" && isAppExpression(variable.init, factories, apps)) {
...
function isAppExpression(value: unknown, factories: Set<string>, apps: Set<string>): boolean {
...
function isEarlyResponseUseStatement(value: unknown, apps: Set<string>): boolean {
...
function middlewareReturnsResponse(value: unknown): boolean {
  ...
  if (middleware?.type !== "ArrowFunctionExpression" && middleware?.type !== "FunctionExpression") {
    return false;
  }
...
function containsResponseReturn(value: unknown): boolean {
  ...
  if (node.type === "ReturnStatement") {
    return containsResponseReturn((node as NodeView & { argument?: unknown }).argument);
  }
  if (
  ...
  return Object.entries(node).some(
    ([key, child]) =>
      key !== "span" &&
      (Array.isArray(child) ? child.some(containsResponseReturn) : containsResponseReturn(child)),
  );
}
...
function isContextResponseCall(value: NodeView): boolean {
  ...
  return (
    ...
    new Set(["body", "html", "json", "notFound", "redirect", "text"]).has(property.name)
  );
}
