...
export function normalizeBasePath(basePath: string): string {
  if (!basePath.startsWith("/")) {
    throw new Error(`[daroyan] basePath must start with "/": ${JSON.stringify(basePath)}`);
  }
...
export function companionFile(root: string, outputDirectory: string, routeFile: string): string {
  ...
  const name =
    relativeFile
      ...
      .split(sep)
      .at(-1) ?? "index";
  return resolve(outputDirectory, "types", dirname(relativeFile), "+types", `${name}.d.ts`);
}
